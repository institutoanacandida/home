...
function Get-StyleBlock {
    ...
    <style id="global-styles">
        ...
        .skeleton-card {
            height: 420px; background: #f5f5f5; border-radius: 1.5rem;
            ...
            try {
                ...
                if (data.status === 'ok') {
                    ...
                    data.items.slice(0, 3).forEach(item => {
                        ...
                        article.style.cssText = 'display: flex; flex-direction: column; cursor: pointer;';
                        ...
                        const category = item.categories && item.categories.length > 0 ? item.categories[0] : 'Psicologia';
                        ...
                        if(!thumb) thumb = 'https://images.unsplash.com/photo-1518331647614-7a1f04cd34cf?q=80&w=1200';

                        article.innerHTML = `
                            <div style="width: 100%; height: 240px; border-radius: 1.5rem; overflow: hidden; margin-bottom: 2rem; background: var(--brand-100);">
                                <img src="${thumb}" alt="${item.title}" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s ease;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                            </div>
                            <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem; font-size: 0.875rem;">
                                <span style="color: var(--brand-500); font-weight: 700;">${category}</span>
                                <span style="color: #ccc;">•</span>
                                <span style="color: #666;">${date}</span>
                            </div>
                            <h3 style="font-size: 1.5rem; color: var(--brand-900); margin-bottom: 1rem; line-height: 1.3;">${item.title}</h3>
                            <p style="color: #555; line-height: 1.6; margin-bottom: 1.5rem; font-size: 1rem; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">${item.description.replace(/<[^>]*>?/gm, '').substring(0, 120)}...</p>
                            <span style="color: var(--brand-500); font-weight: 700; text-decoration: underline; text-underline-offset: 4px;">Ler na fonte original</span>
                        `;
                ...
                console.error('Erro ao carregar o feed:', p);
                feedContainer.innerHTML = '<p style="grid-column: 1/-1; text-align: center; color: #666; padding: 2rem;">Não foi possível carregar os artigos dinâmicos no momento.</p>';
            }
        }
